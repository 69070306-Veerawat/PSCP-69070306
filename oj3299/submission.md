...
```text
input: range for one cycle, num of block
output: total column need
constraint: need to find very complicated solution
```
...
```text
s1: receive input
s2: use for loop to calculate how many column it need
s3: print answer
```
...
```text
nothing like original plan trying to use loop is overly complicated change the approach by find the equation instead
```
