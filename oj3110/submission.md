...
```text
INPUT: source, destination(string)
weight(float)
output: overall price
constraint: what happen is source is same as destination
```
...
```text
step1:receive input
step2:check which condition it is
...
```text
same plan as start write if else condition for all condition then calculate according to given conditon and print it
```
