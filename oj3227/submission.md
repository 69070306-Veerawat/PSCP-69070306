...
```text
INPUT: the short version of card
output: full version of card
constraints: there are many type of card
```
...
```text
s1: receive input
s2: check if front of card wanted is string or num(king,quuen,10,9) then add it to answer string
s3: add the back of card(spade,heart,etc...) by check what input is
s4 print answer
```
...
```text
same as original plan but add .upper to check properly
```
