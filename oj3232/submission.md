...
```text
input: distance per jump, distance need
output: num of jump need(if reach distant need)
constraints: distance per jump decrease 2 per each jump
```
...
```text
s1 receive input
s2 run while loop as long as distance need is not reach or frog are too tired too jump
s3 jump distance -2 per each loop and start count how many jump is need
s4 print num of jump if reach destination or print -1 if destination is not reach
```
...
```text
what if destination is not reach
```
