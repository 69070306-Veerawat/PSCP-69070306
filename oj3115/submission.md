...
```text
input:num of store and time
start, stop of each store
time that wanted to check
output:store that open in each time zone
constraint: this problem use minute which is max at 1440 perday
```
...
```text
step1:receive all input
step2:create list of [0] * 1445
step3:receive open and close time and restore in previous list by add 1 or -1 at open point and close point
step4:create another list of [0] * 1445
step5:run for loop and change new list by += old list
step6:receive time that need to check
step7:check at that specific index of the new list
...
```text
same as the original plan but need to keep changing the format to make it work
```
...
```text
check three store with different open-close time
```
...
```text
check if overlapping time work correctly
```
...
```text
check it time reach 1440 is working correctly
```
