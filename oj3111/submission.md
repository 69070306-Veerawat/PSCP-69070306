...
item
output:total price
constraint:need only two position decimal
```
...
step2: calculate total price before discount
step3:check condition and print the correct answer
```
...
```text
plan remain the same but have to change from using float to import decimal because need to round up specific position
```
