...
```text
find prime number condition in the most simplify form as possible
```
...
```text
ai give me insight about the best way to check if a number is prime number or not instead of a bunch of if else condition
```
...
```text
rewrite the code by using new formula
```
