...
```text
input starting zone
output least number of move that can go to the top
constraint need to find complex equation
```
...
```text
s1 receive int(input())
s2 find all the answer for input = 1-25
s3 print the answer
```
...
```text
final code change entirely from plan
s1 find which row it start from 
s2 check if start zone เป็นสามเหลี่ยมควำ่หรีอหงาย (by using modulus)
if pyramid % 2:
    ans = (2 * row) - 2
else:
    ans = (2 * row) -3
...
```text
check with number greater then 100
```
